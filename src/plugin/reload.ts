...
<code>${mainPrefix}mem</code> - 查看内存状态`;
  ...
  cmdHandlers: Record<string, (msg: Api.Message) => Promise<void>> = {
    ...
    restart: async (msg) => {
      await msg.edit({ 
        text: "🔄 <b>正在通过PM2重启程序...</b>\n\n<i>如果PM2未安装或配置错误，此操作可能会失败</i>", 
        parseMode: "html" 
      });
      ...
      try {
        ...
        // 执行PM2重启命令
        const result = await execAsync("pm2 restart telebox");
        console.log('[ReloadPlugin] PM2 restart result:', result.stdout);
        ...
        await msg.edit({
          text: `❌ <b>PM2重启失败</b>\n\n` +
                `🔧 <b>错误信息:</b> ${errorMessage}\n\n` +
                `📝 <b>建议:</b> 确保PM2已安装并配置正确，应用名称为 'telebox'`,
          parseMode: "html"
        });
      }
    },
